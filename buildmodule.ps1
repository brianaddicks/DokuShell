...
$OutputFile      = $ScriptPath + "\" + "dokushell.psm1"
$ManifestFile    = $ScriptPath + "\" + "dokushell.psd1"
$DllFile         = $ScriptPath + "\" + "dokushell.dll"
$CsOutputFile    = $ScriptPath + "\" + "dokushell.cs"

###############################################################################
# Create Manifest
$ManifestParams = @{ Path = $ManifestFile
                     ModuleVersion = '1.0'
                     RequiredAssemblies = @('dokushell.dll','System.Web')
                     Author             = 'Brian Addicks'
                     RootModule         = 'DokuShell.psm1'
                     PowerShellVersion  = '4.0' }
                     #FormatsToProcess   = @('dokushell.format.ps1xml') }
...
$NameSpaceStartRx = [regex] 'namespace DokuShell {'
...
if ($PushToStrap) {
    ...
    $CreateZip = ZipFiles -ZipFilePath "$PSScriptRoot\dokushell.zip" -SourceFiles $FilesToZip -Force
    $StageFolder = '\\vmware-host\Shared Folders\Dropbox\strap\stages\dokushell\'
    $Copy = Copy-Item "$PSScriptRoot\dokushell.zip" $StageFolder -Force
}